...
class ARQueryRule
{
    ...
    public function  __construct(&$ARQRNode, &$ER, &$attributes, &$IMs, DBAParser $DBAP, BBAParser $BBAP)
    {
        $this->ARQRNode = $ARQRNode;
        $this->ER = $ER;
    ...
    public function parse()
    {
        ...
        // init antecedent
        $antId = $this->XPath->evaluate('AntecedentSetting', $this->ARQRNode)->item(0)->nodeValue;
        if (!empty($antId)) {
            $this->antecedent = $this->parseElement($antId, 1, '');
        }

        // init consequent
        $consId = $this->XPath->evaluate('ConsequentSetting', $this->ARQRNode)->item(0)->nodeValue;
        if (!empty($consId)) {
