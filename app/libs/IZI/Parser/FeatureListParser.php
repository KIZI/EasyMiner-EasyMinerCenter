...
class FeatureListParser {
  ...
  protected function parseSpecialIMs() {
    return array('specialInterestMeasures' => $this->parseSpecialIMTypes());
  }


  protected function parseSpecialIMTypes() {
    $array['types'] = array();
    foreach ($this->XPath->evaluate('BuildingBlocks/SpecialInterestMeasures/Types/Type') as $t) {
      $name = $this->XPath->evaluate('Name', $t)->item(0)->nodeValue;
      $localizedName = '';
      if ($LN = $this->XPath->evaluate('LocalizedName[@lang="' . $this->lang . '"]', $t)->item(0)) {
        $localizedName = $LN->nodeValue;
