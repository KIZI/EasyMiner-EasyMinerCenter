...
class QueryByARSerializer
{
    ...
    public function serialize($json)
    {
        ...
        // Serialize rule
        $associationRule = $this->parseAssociationRule($json->rule0);
    ...
    protected function parseAssociationRule($rule) {
    ...
    protected function createBasicStructure()
    {
        ...
        $document->setAttribute("mode", "QueryByAssociationRule");
        ...
        $this->associationRules = $root->appendChild($this->output->createElement("QueryByAssociationRule"));
    }
    ...
    protected function createDbaSetting($connective, $attributes, $level, $cedentIds)
    {
        ...
        if (!$this->isLiteral($connective)) {
            ...
            $connective = $attribute->sign === 'positive' ? self::$FORCE_DEPTH_BOOLEAN : "Negation";
            $dbaSetting->setAttribute("connective", $connective);
        }
